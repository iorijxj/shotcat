...
日常制作只使用 `web/` 工作台：<http://127.0.0.1:5273>。它是本仓库当前的产品界面，也是功能验证和反馈的唯一页面基准。

`app/front/` 保留的是旧 Studio 管理界面，默认端口 `7788`，用于维护和历史兼容；它的页面结构与 `web/` 不同，不能用来判断日常工作台是否更新。

Git 只同步代码，不同步本地项目数据、生成图片和密钥。每台机器的 `app/backend/jellyfish.db`、`app/backend/local-storage/` 与 `.env` 均独立保存；要看到同一项目与同一图片，需要另行导入项目数据和资源。
...
| `app/front/` | 旧 Studio 管理前端，仅用于维护与历史兼容，默认端口 `7788`。 |
